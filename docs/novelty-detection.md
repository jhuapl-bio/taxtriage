...
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `mmseqs2`   | `MMSEQS_TAXONOMY`, a translated-search LCA (MMseqs extracts ORFs from the contigs internally).                                      | MMseqs seqTaxDB **name** (e.g. `Kalamari`, `UniProtKB`) or a local seqTaxDB directory.                                                                                                                                                                               | `--novelty_db_cache` (opt-in; default none)         |
| `kaiju`     | `KAIJU`, a protein-level translated search (self-translates the contigs). Run mode is `greedy` or `mem` via `--novelty_kaiju_mode`. | A kaiju index **alias** (`test`, `viruses`/`viral`, `nr`, `nr_euk`, `refseq`, `refseq_nr`, `refseq_ref`, `progenomes`, `fungi`, `plasmids`, `rvdb`), an exact `*.tgz`, a URL, or a local dir.                                                                        | `--novelty_kaiju_db_cache` (opt-in; default none)   |
| `bracken`   | `KRAKEN2_NOVELTY` → `BRACKEN`, meaning kraken2 over the contigs, then bracken re-estimates count-weighted abundance.                | A kraken2/bracken index **alias** (`standard`/`standard_8gb`/`standard_16gb`, `viral`, `minusb`, `pluspf[_8gb\|_16gb]`, `pluspfp[_8gb\|_16gb]`, `core_nt`, `gtdb`, `eupathdb`, `ncbi_reference`; `k2_` prefix optional), an exact `*.tar.gz`, a URL, or a local dir. | `--novelty_kraken2_db_cache` (opt-in; default none) |

Aliases are auto-downloaded from the configured buckets (`--novelty_kaiju_db_baseurl`, `--novelty_kraken2_db_baseurl`). By default the download lands in the task work directory, exactly like the main `--db`, and nothing is cached across runs. Caching is opt-in: pass `--db_cache_dir <base>` (or a per-backend `--novelty_*_db_cache`) pointing at somewhere the executor can actually reach — a bucket prefix such as `s3://my-bucket/dbs` on cloud, a shared mount on a cluster, any folder locally. A genuine local checkout still uses `<projectDir>/dbs/<backend>` automatically.

> **Why the default is "no cache".** An auto-derived cache path is a local POSIX path on the head node, which cloud workers cannot reach. On Seqera with Fusion it fails hard — `Unexpected path for Fusion script launcher: /nftass/scratch/.../dbs/kaiju/viral` — which is why `--novelty_db viral` used to die where `--db` never did. A local cache path is also ignored automatically whenever the work directory is a bucket URI.

The kaiju `test` alias is a tiny git-LFS-hosted index for CI and `-profile test`; it always lives in `work/` like any other test download. The bracken backend additionally honours `--novelty_bracken_readlen` (default `100`, the kmer-distribution read length) and `--novelty_bracken_level` (default `S` = species).
...
| `--novelty_db_cache`           | _unset_              | Opt-in `storeDir` cache for the mmseqs2 download.                                                                                                    |
| `--novelty_kaiju_db_cache`     | _unset_              | Opt-in `storeDir` cache for the kaiju download.                                                                                                      |
| `--novelty_kraken2_db_cache`   | _unset_              | Opt-in `storeDir` cache for the bracken/kraken2 download.                                                                                            |
