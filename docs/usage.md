...
| `--trim` <true|false>`                         | Whether the reads should be trimmed prior to downstream processing. Accepts `"true"` or `"false"`. Defaults to `false`.                                                                                                       |
| `--type <type>`                               | Sample type or classification (e.g., `clinical`, `environmental`, or any custom string). Optional — can be used for filtering or labeling during analysis.                                                                   |
