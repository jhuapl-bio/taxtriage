...
❗ **Note**: the `sample` column is used to identify samples across multiple lines. If you specify a positive or negative value it is related to the row with the same samplename. Those samples will then be compare to ANY of the rows with the same sample name that do not have a positive or negative control specified. If you specify a positive or negative value for a sample name that is not present in any other row, it will be ignored and no control will be used for that row.
...
| `positive`           | OPTIONAL, but if you have a positive lab-control you'd like to compare results you can place that sample (the value in the sample column for the control) name for any non-lab controls. See example samplesheet as a reference                                                                                                                                                                                                                                                                                                                                                                             |
| `negative`           | OPTIONAL, but if you have a negative lab-control you'd like to compare results you can place that sample (the value in the sample column for the control) name for any non-lab controls. See example samplesheet as a reference                                                                                                                                                                                                                                                                                                                                                                             |

In Addition, you can provide metadata in one of two ways:

1. Provide a `--meta` parameter with the path to a metadata CSV file. This file must have a `sample` column that matches the `sample` column in the samplesheet. The metadata will be used in the report and can be used for filtering and other downstream analysis.
2. Add metadata directly into the samplesheet as additional columns. These columns will be added to the report and can be used for filtering and other downstream analysis. The metadata CSV file provided in `--meta` will take priority over any additional columns in the samplesheet if there are any conflicts.

| Column               | Used in Multi Run Analysis Report | Description                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sample`             | Yes                               | **MANDATORY** Custom sample name. This entry will be identical for multiple sequencing libraries/runs from the same sample. Spaces in sample names are automatically converted to underscores (`_`).                                                                                                                                                                                                                      |
| `run_id`             | Yes                               | OPTIONAL Run identifier. This can be used to group samples by run in the report. If not provided, all samples will be grouped together.                                                                                                                                                                                                                                                                                   |
| `latitude`           | Yes                               | OPTIONAL Latitude of the sample collection site. This can be used for geospatial analysis in the report.                                                                                                                                                                                                                                                                                                                  |
| `longitude`          | Yes                               | OPTIONAL Longitude of the sample collection site. This can be used for geospatial analysis in the report.                                                                                                                                                                                                                                                                                                                 |
| `collection_time`    | Yes                               | OPTIONAL Time of sample collection. This can be used for temporal analysis in the report `report/all.odr.html`. Format must be: `YYYY-MM-DD HH:MM:SS` e.g. `3/12/24 9:45`                                                                                                                                                                                                                                                 |
| `....Other Metadata` | Yes                               | OPTIONAL Any other metadata columns can be added to the samplesheet or provided in a separate metadata CSV file. These will be included in the report and can be used for filtering and other downstream analysis. The metadata CSV file provided in `--meta` will take priority over any additional columns in the samplesheet if there are any conflicts. Examples would be things like: altitude, depth, comments, etc |

An [example samplesheet](../examples/Samplesheet.csv) has been provided with the pipeline alongside some demo data.

An [example ODR](https://jhuapl-bio.github.io/taxtriage/) has also been provided with the pipeline to show how the metadata is used in the report and how the different sample types are displayed. See the **Sample Metadata & Map tabs**.
...
❗ Note that commensal or potential pathogens will by default be hidden in the PDF but available in the **.paths.txt** file in the reports folder [see here for more info](#final-text-output-location). They can also be enabled, in the PDF, with `--show_potentials --show_commensals`
...
⚠️ Negative and Positive controls are not possible in this mode.

⚠️ This process will download the pathogen fasta file on the first run. You can also download the fasta file directly like:
...
```
nextflow run main.nf -profile mce_seqera,docker -resume --fastq_1 examples/data/iss_reads_R1.fastq.gz --fastq_2 examples/data/iss_reads_R2.fastq.gz --type blood --sample test --reference_fasta pathogens.fasta
```
...
| `-latest`                                             | Use the latest revision (`-r`). If you want to autopull the latest commit for a branch from the repository, specify this. Used in Basestack and the Cloud (default toggle). |
...
| `--minq <number>`          | Minimum quality desired in samples. Disabled if you run `--skip_fastp`. Default is 7 for Oxford Nanopore, 20 for Illumina.                                                     |
...
| `--downsample`             | Uses the BBNorm submodule within the bbmap suite (JGI) to reduce redundant reads for downstream analysis. Useful for deep sequencing samples (e.g. 40 GB of reads per sample). |
| `--decompress_pre_megahit` | Some HPC environments cannot use MEGAHIT with gz files; enable if this is the case or you experience input/output errors at this step.                                         |

## Taxonomic Classification and Databases

| Parameter                              | Description                                                                                                                                                                                                                                                                                                                                                                                |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--db <path_to_kraken2_database>`      | Database to be used. If `--low_memory` is called it will read the database from the filesystem. If not called, it will load it all into memory first, so ensure available memory is enough to hold the database. If using with `--download_db`, choose from download options `{minikraken2, flukraken2}` instead of using a path. [See here for a full list](#supported-default-databases) |
...
| `--top_per_taxa "<taxid:amount:rank>"` | One or more 3-element values for the minimum taxa at a rank. Example: `"10239:20:S 2:20:S"` means minimum 20 virus species and 20 bacteria species. Rank codes: `G`, `S`, `P`, `F`, `O`, `C`.                                                                                                                                                                                              |
| `--top_hits_count`                     | Minimum taxa to filter out per rank level. If `top_per_taxa` is specified, whichever is larger takes priority.                                                                                                                                                                                                                                                                             |

## Reference Selection and Assemblies

| Parameter                      | Description                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--reference_fasta <filepath>` | Location of a single reference FASTA, multiple files (separated by spaces inside quotes), or a directory of FASTA files to run alignment on rather than or in addition to downloading references from NCBI from post-K2 or manual organism taxids/names. Header format must be `>accession description` with a space after the accession. Example: `>NC_003663.2 Cowpox virus, complete genome` |
...
| `--assembly`                   | Local or remote location to pull the assembly summary RefSeq file for pulling genomes automatically. Useful if you don't have internet or NCBI has issues pulling that file at the time.                                                                                                                                                                                                        |
| `--use_denovo`                 | Perform the de novo assembly process. Currently this is recommended as MEGAHIT for short reads and FLYE for long reads. Disabled by default.                                                                                                                                                                                                                                                    |
| `--use_megahit_longreads`      | If running de novo assembly, use MEGAHIT for long reads. Default is disabled = FLYE.                                                                                                                                                                                                                                                                                                            |
| `--reference_assembly`         | Perform the reference-based assembly on reads against top hit assemblies or assemblies provided as a local file. Performs variant analysis by default. Disabled by default.                                                                                                                                                                                                                     |
| `--generate_iss`               | Create a series of ISS (InSilicoSeq) MiSeq reads for all samples to benchmark against each non-control sample.                                                                                                                                                                                                                                                                                  |
| `--generate_nanosim`           | Generate NanoSim reads (ONT) for all non-control samples. Metrics are displayed just like the ISS version.                                                                                                                                                                                                                                                                                      |
| `--nanosim_training`           | Required if using NanoSim. Provide the path of the model in the NanoSim directory.                                                                                                                                                                                                                                                                                                              |
| `--thresholds_json`            | A JSON file containing info on the "best" weights to use for a given clinical and platform combo. Default is at [here](../assets/sampletype_best_thresholds.json).                                                                                                                                                                                                                              |

## Alignment and Mapping

| Parameter                          | Description                                                                                                                                                                                                                               |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--minmapq <number>`               | Minimum mapping quality desired in sample alignments. Default is 5.                                                                                                                                                                       |
| `--min_mapq_host <number>`         | Minimum MAPQ score to remove host reads. Higher values are more strict. Only used if you provide `--remove_reference_file`. Default is null / no strictness.                                                                              |
| `--mmap2_window <number>`          | Window size to use for alignment with minimap2. Default is 10 if disabled.                                                                                                                                                                |
...
| `--no_split_prefix`                | When using minimap2, specify you do not want to use `split-prefix`, reducing RAM savings but potentially speeding runtime and reducing I/O. Default mode uses split-prefix.                                                               |
| `--loose`                          | Less sensitive mode for false-positive removal, attempts to compare signatures during the TASS calculation across just the queries aligned rather than whole genomes. Slower and more I/O intensive. Useful if many variants are present. |

## Host Removal

| Parameter                          | Description                                                                                                                                                           |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--genome`                         | Pull one of the pre-made iGenomes databases for host removal. Use the key value such as `GRCh37`, `BDGP6`, `GRCz10`, etc.                                             |
| `--remove_reference_file`          | Remove all reads that align to a set of accessions in a single FASTA file.                                                                                            |
| `--include_singletons_hostremoval` | Default is FALSE. For paired reads only. During host removal, retain singletons by passing this variable. Ignored if running paired-end data with MEGAHIT or diamond. |

## Pathogen Discovery and Reporting Filters

| Parameter                     | Description                                                                                                                                                                              |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--pathogens`                 | Optional. Enable pathogen discovery mode. See `assets/pathogen_sheet.txt` for the default sheet. You can use your own designation if it is in the same format as the provided example.   |
| `--get_pathogens`             | TRUE/FALSE, use the pathogens sheet FASTA file instead of K2's top hits. Default is FALSE.                                                                                               |
| `--remove_taxids <numbers[]>` | List of one or more taxids to remove from the Kraken report prior to downstream analysis. Use `"9606"` for human reads.                                                                  |
| `--compress_species`          | TRUE/FALSE, default FALSE. Compress all taxonomies in the ODR to the species level.                                                                                                      |
| `--show_commensals`           | TRUE/FALSE, show or do not show commensals in the final report. Default is do not show.                                                                                                  |
| `--show_potentials`           | TRUE/FALSE, show or do not show potential pathogens in the final report. Default is do not show.                                                                                         |
| `--show_unidentified`         | TRUE/FALSE, show or do not show unknown or unannotated pathogens in the final report. Default is do not show.                                                                            |
| `--add_irregular_top_hits`    | Adds irregular distributions (outside percentile z-score) to top hits. Default is false and only pathogens and top abundance hits are considered.                                        |
| `--distributions`             | By default uses this [file](https://github.com/jhuapl-bio/taxtriage/blob/main/assets/taxid_abundance_stats.hmp.tsv.gz). You may provide your own `.tsv.gz` or `.tsv` distributions file. |
| `--min_conf <number>`         | Minimum confidence to appear within the PDF report (ODR).                                                                                                                                |
| `--negative`                  | Set all non-control samples to compare to the negative lab control based on the sample name specified in the samplesheet. Does not work if you provide `--fastq_1` and/or `--fastq_2`.   |
| `--positive`                  | Set all non-control samples to compare to the positive lab control based on the sample name specified in the samplesheet. Does not work if you provide `--fastq_1` and/or `--fastq_2`.   |

## TASS Scoring and Threshold Parameters

| Parameter                           | Description                                                                                                                 |
| ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `--gini_weight <number>`            | Gini weight for the TASS score. Measures how spread out depths/coverages are in an alignment to an organism.                |
| `--breadth_weight <number>`         | Minimum log-based breadth weight for TASS score calculation.                                                                |
| `--minhash_weight <number>`         | Weight for minhash filtering. Combination of percent reads removed and percent breadth new divided by 2.                    |
| `--mapq_weight <number>`            | MAPQ weight for the TASS score. If filtering on 20+, this will likely all be 1 or close to 1. Disabled by default.          |
| `--hmp_weight <number>`             | TASS score weight for HMP percentile. Recommend disabling if sample is unknown.                                             |
| `--disparity_score_weight <number>` | Weight for giving more of a bonus for more prevalent organisms. Unused by default.                                          |
| `--dispersion_factor <number>`      | TASS score dispersion factor. Measures how spread out depths/coverages are in an alignment to an organism.                  |
| `--reward_factor <number>`          | TASS score reward factor. Measures how spread out depths/coverages are in an alignment to an organism.                      |
| `--disable_auto_weights <boolean>`  | Disable use of clinically derived best cutoffs and weights. Otherwise defaults or manually defined parameters will be used. |
| `--ani_threshold`                   | If ANI matrix is enabled, sets the ANI threshold for a strain or species to be considered high ANI. Default is 0.95 (95%).  |
...
| `--skip_matrix`                 | Skip ANI similarity matching for the final report. Improves compute time.                                                                      |
| `--integrate_strains_table`     | Boolean TRUE/FALSE (default). If false, strains are listed in a separate table. If true, the table is merged into the same row.                |
| `--sort_alphabetical <boolean>` | In the final PDF, sort tables alphabetically by organism name instead of High Consequence then TASS score.                                     |
| `--no_subkey <boolean>`         | In the final PDF, do not organize the table into separate species and strain tables; only show each organism hit regardless of strain/species. |
...
:warning: Be aware that some commands may require specific files that were present during the initial run. If those files are not present, the command may fail. Please ensure all necessary input files are available in the working directory before rerunning any commands.

:warning: Nextflow will attempt to connect to AWS as an anonymous user by default. If you are using private S3 buckets or resources that require authentication, please ensure your AWS credentials are properly configured in your environment to avoid access issues.
...
| centrifuge refseq  | [Download](https://genome-idx.s3.amazonaws.com/centrifuge/p_compressed%2Bh%2Bv.tar.gz)                                               | 7GB    |
| centrifuge LLNL NT | Run `curl 'https://genome-idx.s3.amazonaws.com/centrifuge/llnl/nt_wntr23/nt_wntr23_filt.cf.7z.[001-071]' -O` in cmd line to download | >400GB |
...
- Centrifuge databases [here](https://benlangmead.github.io/aws-indexes/centrifuge)
