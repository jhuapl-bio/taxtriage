...
| `--save_classified_fastq`              | Save the Kraken2 classified reads as FASTQ and use them downstream - classified reads are passed on, unclassified reads are discarded. Default: `FALSE`.                        |
| `--save_k2_read_assignment`            | Save the Kraken2 per-read classification output file. Default: `FALSE`.                                                                                                         |
...
| `--assembly_summary_refseq <path>`                        | Local path to a pre-downloaded NCBI **RefSeq** assembly summary file (`assembly_summary_refseq.txt`). Alias of `--assembly`; when set, the RefSeq summary is not downloaded.                                                                                       |
| `--assembly_summary_genbank <path>`                       | Local path to a pre-downloaded NCBI **GenBank** assembly summary file (`assembly_summary_genbank.txt`). When set, the GenBank summary is never downloaded (even with `--enable_genbank`); the local file is used for taxid lookups and spike-in `GCA_` references. |
...
| `--sim_subsample`                                         | Subsample the master synthetic dataset into a defined read-count series (spike-in / dilution datasets). See [In-Silico](in-silico.md#subsampling-spike-in--dilution-series-datasets).                                                                              |
| `--sim_subsample_mode <consistent\|randomized>`           | `consistent` = nested prefixes (each dataset is a superset of the smaller one); `randomized` = each dataset independently sampled. Default `randomized`.                                                                                                           |
| `--sim_series_counts <list>`                              | Explicit read-count list for the series, e.g. `'100,500,1000,5000'`. One dataset per value.                                                                                                                                                                        |
| `--sim_series_start / --sim_series_step / --sim_series_n` | Generator alternative to `--sim_series_counts`: evenly spaced counts (`start + i*step` for `n` datasets).                                                                                                                                                          |
| `--sim_series_replicates <int>`                           | Dilution series: number of datasets produced per read count (randomized mode). Default `1`.                                                                                                                                                                        |
| `--sim_subsample_seed <int>`                              | RNG seed for reproducible randomized subsampling. Default `42`.                                                                                                                                                                                                    |
| `--sim_keep_subsampled_fastq`                             | Publish the subsampled FASTQs. Off by default: only index files + manifest are kept, and FASTQs are reconstructable via `bin/reconstruct_insilico_reads.py`.                                                                                                       |
| `--background_reads <path>`                               | Dilute a real FASTQ (single-end / R1) into the same read-count series instead of/alongside synthetic reads. Enables the natural-background dilution series. See [In-Silico → Natural background](in-silico.md#natural-background-dilution-series-real-reads).      |
| `--background_reads2 <path>`                              | Background R2 FASTQ for paired-end input; omit for single-end.                                                                                                                                                                                                     |
| `--background_from_sheet`                                 | Take background sources from the samplesheet's `background` column instead of (or as well as) `--background_reads`. Inert unless a simulation or spike-in param is also set.                                                                                       |
| `--spikein_sheet <path>`                                  | CSV/TSV/XLSX of `accession, count, replicates[, level]`. Switches the series from **diluting** the background to **spiking organisms into** it at the stated read counts. See [In-Silico → Spike-in series](in-silico.md#spike-in-series-fixed-background).        |
| `--spikein_background_depth <int>`                        | Subsample the background to this many reads/pairs before spiking, so every dataset sits on a background of known size. Default: use the whole background.                                                                                                          |
| `--spikein_seed <int>`                                    | RNG seed for choosing which pool reads are spiked. Defaults to `--sim_subsample_seed`.                                                                                                                                                                             |
| `--spikein_pool_factor <int>`                             | Simulate this many times the largest requested count per organism, so replicates draw different reads from the same pool. Default `3`.                                                                                                                             |
| `--background_platform <ILLUMINA\|OXFORD\|PACBIO>`        | Platform for the background sample. Default: ILLUMINA if paired, else OXFORD.                                                                                                                                                                                      |
| `--background_name <str>`                                 | Sample id for the background source (prefix for its subsample dataset ids). Default `background`.                                                                                                                                                                  |
...
| `--rescue_multimapped`            | Re-score MAPQ-0 multimapped reads on their own alignment phred instead of dropping them at `--minmapq`. Default: `true`; set `--rescue_multimapped false` for a hard cut. See [Multimapper Rescue](multimapper-rescue.md). |
| `--rescue_min_aln_phred <float>`  | Phred bar for a multimapped read's own alignment, `-10*log10(NM/aligned bases)`. Default: the value of `--minmapq`. 2% divergence is about Q17.                                                                            |
| `--rescue_max_mapq <N>`           | Only alignments at or below this MAPQ are eligible for rescue. Default: `0` (true ties only).                                                                                                                              |
| `--rescue_min_aln_frac <float>`   | Minimum unclipped fraction of the read for a rescue. Default: platform preset - `0.95` Illumina, `0.80` ONT, `0.85` PacBio.                                                                                                |
| `--rescue_max_nm_rate <float>`    | Optional extra ceiling on NM per aligned base (e.g. `0.02`). Default: unset.                                                                                                                                               |
| `--rescue_min_aln_len <N>`        | Minimum aligned query length (bp) for a rescue. Default: `50`.                                                                                                                                                             |
| `--rescue_require_proper_pair`    | Paired-end only: restrict rescues to reads flagged properly paired (`0x2`).                                                                                                                                                |
| `--rescue_counts_as_highmapq`     | Count rescued reads toward `highmapq_fraction`, which scales breadth and Gini. Off by default.                                                                                                                             |
...
| `--novelty_db_cache <path>`                | _unset_              | Opt-in `storeDir` cache for the mmseqs2 download (unset = download into the work dir, like `--db`).                                                                                                                                                                                                                                                                                                                                                                 |
| `--novelty_db_prefix <str>`                | `seqTaxDB`           | Base name of the mmseqs2 database files inside the db directory.                                                                                                                                                                                                                                                                                                                                                                                                    |
| `--novelty_kaiju_db_cache <path>`          | _unset_              | Opt-in `storeDir` cache for the kaiju download (unset = download into the work dir, like `--db`).                                                                                                                                                                                                                                                                                                                                                                   |
| `--novelty_kraken2_db_cache <path>`        | _unset_              | Opt-in `storeDir` cache for the bracken/kraken2 download (unset = download into the work dir, like `--db`).                                                                                                                                                                                                                                                                                                                                                         |
...
## Report Organism-QC Flags

Per-**detection** criteria (one organism in one sample) that seed the interactive report's **Organism QC / Flags** rule set. Report _defaults only_ - nothing is removed from any run output. A matching row is highlighted in the Summary, Table and Heatmap or, with `--report_org_flag_action hide`, removed from the report's views (reversible from the sidebar dropdown). Every rule can be edited live in the report.

Each of `--report_org_flag_min_reads`, `--report_org_flag_min_tass` and `--report_org_flag_ani` becomes its own rule (a row is flagged when any matches). `--report_org_flag_genus` **scopes** all of them to the listed genera; given on its own, it flags every organism in those genera.

| Parameter                                       | Description                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--report_org_flag_min_reads <n>`               | Flag an organism with fewer than `n` reads aligned.                                                                                                                                                                                                                                                                                                                            |
| `--report_org_flag_min_tass <tass>`             | Flag an organism whose TASS score is below `tass`.                                                                                                                                                                                                                                                                                                                             |
| `--report_org_flag_ani <pct>`                   | Flag an organism sharing at least `pct` % ANI with another hit in the same sample. Needs `--enable_matrix`; partners are only recorded at or above `--ani_threshold`.                                                                                                                                                                                                          |
| `--report_org_flag_ani_partner <stronger\|any>` | `stronger` (default): only a partner with more reads (then higher TASS) counts, so of a near-identical pair only the weaker one is flagged. `any`: both are.                                                                                                                                                                                                                   |
| `--report_org_flag_genus <genera>`              | Comma-separated genera that scope every criterion above.                                                                                                                                                                                                                                                                                                                       |
| `--report_org_flag_criteria <spec>`             | Free-form rules: `;` between rules, `&` between the conditions of a rule (all must hold), each `field:op:value`. `field` is a detection column or one of `ani_max` `ani_partners` `genus_reads` `genus_share` `genus_rank` `genus_members` `sample_read_share` `k2_ratio` `prevalence` `lineage`. Operators: those of `--report_flag_metadata` plus `in` / `!in` (comma list). |
| `--report_org_flag_action <flag\|hide>`         | `flag` highlights (default); `hide` also removes matching rows from every view.                                                                                                                                                                                                                                                                                                |
| `--report_org_flag_view <all\|hide\|only>`      | Which rows the report opens on: `all` (flagged highlighted), `hide` (flagged hidden) or `only` (nothing but the flagged rows).                                                                                                                                                                                                                                                 |

A `--report_flag_rules` JSON file may also carry an `"organism_rules"` block - `{"view": "hide", "rules": [{"action": "hide", "conds": [{"field": "Genus", "op": "in", "value": "Streptococcus"}, {"field": "# Reads Aligned", "op": "<", "value": 50}]}]}` - which replaces the parameters above.

Example - hide low-read _Streptococcus_ / _Staphylococcus_ hits, the weaker of any pair sharing ≥ 99 % ANI, and minor members of a genus (`--report_org_flag_genus` scopes only the min-reads / min-TASS / ANI parameters, not `--report_org_flag_criteria`):

```bash
nextflow run . -profile docker \
    --input samplesheet.csv --enable_matrix \
    --report_org_flag_genus "Streptococcus,Staphylococcus" --report_org_flag_min_reads 50 \
    --report_org_flag_criteria "ani_max:>=:99; genus_rank:>:1 & genus_share:<:5" \
    --report_org_flag_action hide
```
