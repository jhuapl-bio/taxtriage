...
HIGH_RANKS = {"genus", "family", "order", "class", "phylum", "superkingdom", "kingdom"}
...
def parse_args(argv=None):
    ...
    p.add_argument("--min-reads", type=int, default=10,
                   help="min reads at a taxon to report it as a genus+ candidate")
...
def main(argv=None):
    ...
    with open(cand_path, "w", newline="") as fh:
        wri = csv.writer(fh, delimiter="\t")
        wri.writerow(["sample", "taxid", "rank", "name", "reads", "frac_of_sample"])
        for (taxid, rank, name), n in sorted(by_taxon.items(), key=lambda kv: -kv[1]):
            if n >= a.min_reads:
                wri.writerow([a.sample, taxid, rank, name, n, f"{n/total:.4f}"])
