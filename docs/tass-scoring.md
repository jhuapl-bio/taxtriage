...
![TASS Confidence Metric Overview](images/confidence_metric.png)
...
Closely related organisms (e.g., _E. coli_ vs _Shigella_) share large regions of their genomes. This means reads from one organism can legitimately align to the other. This is called **cross-mapping**, and it inflates the apparent confidence for organisms that aren't really there.
...
Connected components (found with a breadth-first search, or BFS) define **conflict groups**: clusters of regions across multiple organisms that share enough k-mer content that reads could be assigned to the wrong member.
...
| `--no_read_removal` _(pipeline param)_     | `false` | Disable conflict read removal **entirely for scoring**. Conflict regions and breadth-change stats are still computed and written to disk (reference lengths and reports stay intact), but no reads are removed: coverage, Gini, `numreads` and the full TASS score all derive from the complete original alignment set.                                            |
...
| `--taxid_removal_stats` _(pipeline param)_ | `true`  | In addition to the standard `removal_stats.xlsx`, write a taxid-aggregated `removal_stats_by_taxid.xlsx` in which accessions are grouped by taxid. Requires `--match` (it supplies the accession → taxid mapping). On by default in the pipeline; pass `--taxid_removal_stats false` to skip it.                                                                   |
...
Before any reads are removed, `_count_reads_per_ref()` builds a dict of `{reference: read_count}` for every contig in the BAM. It uses the BAM index statistics (`pysam.AlignmentFile.get_index_statistics()`) which is an O(N_references) operation, meaning no reads have to be iterated over. If the index is unavailable, it falls back to a full single-pass scan.
...
These are benchmarking aids: `--report_metrics` is what you use when tuning weights against a known mock community, and the confusion XLSX is the drill-down for _which reads_ are responsible for a false positive that removal did not clear.
...
**Top hits decision tree (feeds into which organisms reach the scoring stage):**

![TASS Decision Tree](images/TASSDiagram.png)
...
- $m$ = the midpoint, meaning the coverage level at which the sigmoid outputs 0.5, meaning "we're 50/50 on whether this is real." Default is 0.01 (i.e., 1% of the genome covered).
- $s$ = the steepness, which controls how quickly the curve transitions from low confidence to high confidence around the midpoint. A higher number makes the jump sharper. Default is 12,000.
...
> **Where $c$ comes from at species/genus level.** At the strain level $c$ is simply covered bases ÷ reference length. At the species and genus levels it is subject to the roll-up overrides in [11.2](#112-speciesgenus-roll-up-score-overrides). In particular it can be replaced by a **maximum across member accessions**, which is size-gated to avoid a single small scaffold standing in for a whole genome ([11.3](#113-size-eligibility-for-the-representative-coverage-maximum)). Because the sigmoid saturates by ~1.5 % coverage, this input is worth checking first when a species breadth score looks implausibly high next to its reported `Breadth %`.
...
**MAPQ score sigmoid curve:**

![MAPQ Score](images/svgs/mapq_score.svg)
...
![Gini Coefficient Illustration](images/Gini_coeff.png)
...
- $\sigma^2$ (sigma squared) is the **variance** of those midpoints, meaning how spread out they are from the average. If all regions are near the same spot, variance is low. If they're scattered across the genome, variance is high.
...
An organism that loses most of its reads during conflict resolution probably wasn't genuinely present, because those reads belonged to something else. An organism that retains its reads is more likely real.
...
- **`Δ⁻¹ Breadth`** ($B_r$): The ratio of post-removal breadth to pre-removal breadth. Think of it as "what fraction of genome coverage survived after we removed ambiguous reads?" A value of 1.0 means no breadth was lost, which is good because it means the signal was all unique. A value of 0.5 means half the coverage came from reads that could've belonged to a different organism (concerning).
- **`Δ All%`** ($\Delta\%$): The percentage of total reads that were removed during conflict resolution. The Δ (delta) here means "change," specifically how many reads changed by being removed.
...
**Example:** O104:H4 has 60% of group RPM, `raw_minhash` = 0.90, `minhash_confidence` = 0.95:
...
- $f_{\text{obs}}$ is the **observed fraction** of reads belonging to this organism, which is simply its read count divided by the total read count.
...
- $\sigma_{\text{HMP}}$ (sigma) is the **standard deviation** of that abundance, which tells you how much variation is normal.
...
- $p = 1.0$: no change (this is the default, and the score passes through as-is)

The transform is **not** applied uniformly across organisms: a per-record `score_power_scale` derived from ANI grouping determines how much of it each organism receives. See [11.6](#116-ani-aware-score-power-scaling).
...
![Full Score Formula](images/svgs/full_score.svg)

**Disparity score example:**

![Disparity Score](images/svgs/disparity.svg)
...
1. the record is a real group aggregation with union coverage engaged (a `group_covered_bp_override` exists for this group);
...
The optional power transform of Section 9.5 does not apply uniformly. `match_paths.py` pre-computes a per-organism `score_power_scale` that controls how much of the transform each record receives.
...
- `score_power_scale` **does not divide the TASS score**; it only changes an exponent;
- it has no effect whatsoever when `score_power = 1.0` (the default);
- the underlying breadth, minhash, Gini and disparity metrics remain the whole basis of the score.

At the species and genus level the aggregate record normally represents the entire group and receives a scale of `1.0`. If ANI links several nominal parent groups into a larger super-group, proportional scaling is applied at the aggregate level too. That is deliberately conservative, but it does mean extreme similarity across separate taxonomy IDs can still hold back parent-rank recalibration.

### 11.7 Worked example: six near-identical strains

Six strains of one species receive nearly equal counts because most reads map equally well to all six:

```text
Strain 1: 103 reads      Strain 4:  99 reads
Strain 2: 101 reads      Strain 5:  98 reads
Strain 3: 100 reads      Strain 6:  97 reads
```

**Strain level.** No strain dominates, so sibling disparity is weak, strain-specific retention after conflict removal is reduced, and MAPQ is low because the aligner cannot choose a reference. With ~600 reads in the similarity group each strain's proportion is ≈ `100/600` = 0.167, so each receives only about one-sixth of the score-power effect when that transform is enabled (11.6). Again, this does not divide TASS by six.

**Species level.** All six mappings agree on the species, so aggregation retains reads ambiguous only among those six strains (11.1), restores the species-level pre-MAPQ read count and rank-aware breadth (11.2), applies conditional MAPQ relief now that minhash supports the group (11.5), recomputes Gini over aggregate coverage, and calculates a fresh species TASS. **Low or moderate strain scores with a high species score is the correct output here**, not a bug.

**Genus level.** Computed independently with genus-aware retention. If ambiguity spans several species of one genus, low strain / moderate species / high genus is a valid and informative pattern. The parent score reflects support for the shared taxon; it does not assert that every child reference is present.
...
See info in the [insilico testing docs](in-silico.md) for how reads were generated to establish a ground truth of hits post-alignment. When optimizing, the reads are mapped to ground truth by reading the fastq read names, which are formatted like this:
