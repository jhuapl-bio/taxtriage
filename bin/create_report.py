...
class _OutlineCollector:
    """Accumulates PDF outline entries and flushes them in document order.
    ...
    ``FinalizeOutlines``.  Flushing preserves **document order** (the
    registration order) so the PDF sidebar panel mirrors the physical layout
    of the document and every jump lands at the correct position.

    Level-1+ children are kept immediately under their preceding level-0
    parent, exactly as they appear in the document.
    """

    def __init__(self):
        # Each entry: (dest_key, title, level, closed)
        self.entries = []

    def add(self, dest_key, title, level=0, closed=True, sort_key=None):
        # sort_key is accepted for API compatibility but is no longer used.
        self.entries.append((dest_key, title, level, closed))

    def flush_sorted(self, canv):
        """Emit outline entries in document order, preserving parent-child
        nesting, and add them to the canvas outline."""
        # Walk the flat list once: emit every entry directly.
        # ReportLab interprets the level parameter to build the hierarchy —
        # a level-1 entry following a level-0 entry becomes its child.
        for dest_key, title, level, closed in self.entries:
...
class OutlineDest(Flowable):
    """Zero-size flowable that bookmarks the current position and registers
    an outline entry inline (not deferred).

    Uses ``bookmarkHorizontal(key, 0, 0)`` which transforms the origin (0,0)
    through the current CTM to get the correct page-absolute coordinates.
    This works correctly both in the main story flow AND inside table cells
    (where ``canv._y`` is cell-relative, not page-absolute).

    ``addOutlineEntry`` is called immediately in ``draw()`` so the outline
    entry is registered at exactly the same position as the named destination.
    """
    ...
    def draw(self):
        # bookmarkHorizontal transforms (0,0) by the CTM → correct absolute
        # coordinates both in main story and inside table cells.
        self.canv.bookmarkHorizontal(self.key, 0, 0)
        # Register the outline entry immediately so it points to this position.
        self.canv.addOutlineEntry(self.title, self.key,
                                  level=self.level, closed=self.closed)
        # Still notify the collector (kept for API compatibility; flush_sorted
        # is now a no-op so this has no further effect).
...
class AbsoluteAnchorFlowable(Flowable):
    """Zero-size flowable that registers a named destination.

    Uses ``bookmarkHorizontal(name, 0, 0)`` so the destination resolves to
    the correct page-absolute position via the CTM, working correctly both
    in the main story flow and inside table cells.
    """
...
class FinalizeOutlines(Flowable):
    ...
    Outline entries are now registered inline in ``OutlineDest.draw()``, so
    this flowable is a no-op.  It is kept in the story so existing code that
    appends it continues to work without modification.
    """
    ...
    def draw(self):
        # No-op: outline entries are emitted inline by OutlineDest.draw().
        pass
...
def create_combined_sample_table(all_strains, species_group_map, small_style,
    ...
    data_style = ParagraphStyle(
        'DataStyle', parent=small_style, fontSize=9, leading=9,
    ...
    def _base_col_widths(w):
        ...
        if show_control_bar:
            ...
            if show_k2_column and show_ani_column:
                base = [w*0.03, w*0.24, w*0.07, w*0.09, w*0.12, w*0.09, w*0.09, w*0.15]
            elif show_k2_column:
                base = [w*0.03, w*0.28, w*0.08, w*0.11, w*0.13, w*0.11, w*0.14]
            elif show_ani_column:
                base = [w*0.03, w*0.27, w*0.08, w*0.13, w*0.10, w*0.10, w*0.17]
            else:
                base = [w*0.03, w*0.36, w*0.09, w*0.14, w*0.12, w*0.14]
        ...
        # Normalise so columns always fill exactly w (absorbs any rounding drift)
        _total = sum(base)
        if _total > 0:
            base = [c * w / _total for c in base]
        return base
...
def create_low_confidence_table(low_confidence_strains, small_style, show_k2_column,
    ...
    # Match the compact styling used by the organism rows
    header_style = ParagraphStyle(
    'HeaderStyle', parent=small_style, fontSize=8, leading=9,
    fontName='Helvetica-Bold', textColor=colors.HexColor('#1F4E79'),
    alignment=TA_CENTER, wordWrap='CJK')
    ...
    # Narrow color-swatch column on the left; no label
    if show_k2_column:
        headers = ['', 'Sample', 'Organism', 'TASS Score', 'Classifier Reads', 'Aligned Reads']
        col_widths = [
            available_width * 0.03,  # swatch
            available_width * 0.19,  # sample
            available_width * 0.43,  # organism
            available_width * 0.11,  # tass
            available_width * 0.12,  # classifier reads
            available_width * 0.12,  # aligned reads
        ]
    else:
        headers = ['', 'Sample', 'Organism', 'TASS Score', 'Aligned Reads']
        col_widths = [
    ...
    table_data = [[Paragraph(h, header_style) if h else '' for h in headers]]
    ...
    for item in low_confidence_strains:
        ...
        row = [
            '',  # color swatch cell; styled below
            Paragraph(sample_name, data_style),
            Paragraph(strain_name_text, data_style),
            Paragraph(f"{strain.get('tass_score', 0):.1f}", numeric_style),
        ]
        if show_k2_column:
            row.append(Paragraph(f"{strain.get('k2_reads', 0):,.0f}", numeric_style))
        row.append(Paragraph(f"{strain.get('numreads', 0):,.0f}", numeric_style))
        table_data.append(row)

    table = Table(table_data, repeatRows=1, colWidths=col_widths)

    styles = [
        # Header row style: same compact header treatment used in the organism table
        ...
        # Soft grid like the sample section
        ('BOX', (0, 0), (-1, -1), 0.9, colors.HexColor('#C9D8E3')),
        ('INNERGRID', (0, 0), (-1, -1), 0.4, colors.HexColor('#E1E8EE')),
    ]

    # Color swatch only in the first column; no full-row fill
    for idx, item in enumerate(low_confidence_strains, start=1):
        row_color = get_category_color(
            item['strain'].get('microbial_category', 'Unknown'),
            item['strain'].get('annClass', ''),
            alpha=0.25
        )
        styles.append(('BACKGROUND', (0, idx), (0, idx), row_color))
...
def _first_present_number(meta, keys):
    """Return the first numeric metadata value found for any key."""
    for key in keys:
        val = meta.get(key)
...
def collect_run_highlights(samples_dict, args):
    """Compute run-level metrics for the cover-page Run Highlights cards."""
    per_sample_meta = getattr(args, '_input_metadata', {}) or {}
    sample_min_conf = getattr(args, '_sample_min_conf', {}) or {}

    total_reads = 0.0
    aligned_reads = 0.0
    unique_taxids = set()
    unique_genera = set()
    high_cons_taxids = set()

    for sample_name, species_groups in samples_dict.items():
        meta = per_sample_meta.get(sample_name, {}) or {}

        # Total Reads Analyzed = sum(total_reads) across all samples.
        sample_total = _first_present_number(meta, ['total_reads', 'reads_total'])
        if sample_total is None:
            sample_total = sum(float(sg.get('numreads', 0) or 0) for sg in species_groups)
        total_reads += sample_total

        # % Reads Classified = sum(aligned_reads) / sum(total_reads).
        # Do NOT use total_organism_reads here.
        sample_aligned = _first_present_number(meta, ['aligned_reads'])
        if sample_aligned is None:
            sample_aligned = sum(float(sg.get('numreads', 0) or 0) for sg in species_groups)
        aligned_reads += sample_aligned

        min_conf = sample_min_conf.get(sample_name, _DEFAULT_CONF)

        for sg in species_groups:
            # Count genus-like groups from the top-level organism grouping.
            genus_key = str(sg.get('toplevelkey', sg.get('key', '')) or '').strip()
            genus_name = str(sg.get('toplevelname', sg.get('name', '')) or '').strip()
            if genus_key:
                unique_genera.add(genus_key)
            elif genus_name:
                unique_genera.add(genus_name.lower())

            for strain in get_all_strains(sg):
                if not has_min_reads(strain, getattr(args, 'min_reads', 1)):
                    continue

                taxid = str(strain.get('key', '') or '').strip()
                if taxid:
                    unique_taxids.add(taxid)

                tax = strain.get('taxonomy') or sg.get('taxonomy') or {}
                genus = str(tax.get('genus', '') or '').strip()
                if genus:
                    unique_genera.add(genus.lower())

                if strain.get('high_cons', False):
                    passes = strain_passes_with_subkey_promotion(strain, sg, min_conf)
                    if passes and taxid:
                        high_cons_taxids.add(taxid)

    classified_pct = (aligned_reads / total_reads * 100.0) if total_reads else None
    ...
    return {
        ...
        'unique_genera': len(unique_genera),
        'high_consequence': len(high_cons_taxids),
    }


def create_run_highlights_box(samples_dict, args, available_width, styles):
    """Create a boxed Run Highlights section for the report cover page."""
    metrics = collect_run_highlights(samples_dict, args)
    ...
    card_specs = [
        ('Number of Samples', _fmt_int(metrics['num_samples'])),
        ('Total Reads Analyzed', _fmt_int(metrics['total_reads'])),
        ('% of Reads Aligned', _fmt_pct(metrics['classified_pct'])),
        ('Unique Organisms', _fmt_int(metrics['unique_taxids'])),
        ('Unique Genera', _fmt_int(metrics['unique_genera'])),
        ('High Consequence\nPathogens Detected', _fmt_int(metrics['high_consequence'])),
    ]
    card_w = available_width * 0.305
    outer_col_w = available_width / 3.0

    card_h = 0.70 * inch
    inner_row_heights = [0.34 * inch, 0.36 * inch]

    card_rows = []
    for start in (0, 3):
        row = []
        for label, value in card_specs[start:start + 3]:
            label_html = label.replace('\n', '<br/>')
            inner = Table(
                [[Paragraph(value, value_style)], [Paragraph(label_html, label_style)]],
                colWidths=[card_w],
                rowHeights=inner_row_heights,
                hAlign='CENTER',
            )
    ...
    cards = Table(
        card_rows,
        colWidths=[outer_col_w] * 3,
        rowHeights=[card_h + 0.10 * inch, card_h + 0.10 * inch],
        hAlign='CENTER',
    )
    cards.setStyle(TableStyle([
        # This creates whitespace around each boxed card.
        ('LEFTPADDING', (0, 0), (-1, -1), 3),
        ('RIGHTPADDING', (0, 0), (-1, -1), 3),
...
def create_pdf_template(output_path, samples_dict, args):
    ...
    PageTemplate(id="Legend", frames=[legend_frame], onPage=draw_header),
    ...
    'StrainDetailHeading', parent=small_style, fontSize=12,
    textColor=colors.HexColor('#000000'), spaceBefore=10, spaceAfter=4,
    fontName='Helvetica-Bold', alignment=TA_CENTER)
    # ── Cover page: pipeline overview ──────────────────────────────────────────
    story.append(Spacer(1, 0.50 * inch))

    story.append(Paragraph("Organism Discovery Report", cover_title_style))
    story.append(Spacer(1, 0.05 * inch))

    story.append(Paragraph(
        "This Organism Discovery Report (ODR) was generated by TaxTriage, a flexible, containerized bioinformatics pipeline for pathogen detection from untargeted DNA or RNA sequencing data. "
        "TaxTriage supports both short-read (Illumina) and long-read (ONT, PacBio) sequencing platforms and incorporates quality control, organism classification, "
        "read mapping, and a unified confidence metric for all detected organisms.",
        cover_body_style
    ))

    story.append(Paragraph(
        "The ODR provides a consolidated summary of the evidence supporting pathogen identification, including key results from  "
        "the intermediate analyses performed throughout the pipeline. TaxTriage is intended for broad deployment "
        "and early-stage outbreak investigations and is not intended as a standalone diagnostic capability.",
        cover_body_style
    ))
    story.append(Spacer(1,0.15 * inch))

    story.append(Spacer(1, 0.25 * inch))
    story.append(create_run_highlights_box(samples_dict, args, available_width, styles))
    story.append(Spacer(1, 0.10 * inch))
    # Force the legend / explanations onto the next page
    story.append(NextPageTemplate("Legend"))
...
# ── Footer: Column Explanations ───────────────────────────────────────────
    ...
    expl_rows = [
        ...
        ["Aligned Reads", "Number of reads aligned to the organism's reference genome. (%) indicates the proportion of total sample reads aligned to that organism. <b>Aligned read counts may differ from the original mapping results due to post-processing steps that remove potential false-positive alignments.</b>"],
    ...
    for sample_name in sorted(samples_dict.keys()):
        ...
        summary_data.append([
            Paragraph(sample_link, run_summary_sample_style),
    ...
    # ── Index ─────────────────────────────────────────────────────
    story.append(OutlineDest('outline_toc', 'Index', level=0, closed=True, collector=outline_collector))

    story.append(Paragraph("Index", heading_style))
    ...
    for _sir_sample_name in sorted(samples_dict.keys()):
        _sir_bookmark = f"sample_{sanitize_bookmark_name(_sir_sample_name)}"
        _sir_species_groups = samples_dict[_sir_sample_name]
        _sir_total_alignments, _sir_primary_count = get_sample_stats(_sir_species_groups)
        _sir_mc = sample_min_conf.get(_sir_sample_name, _DEFAULT_CONF)
        _sir_visible = []
        for _sg in _sir_species_groups:
            _q = get_qualifying_strains(_sg, args, _sir_mc, min_reads=1)
            if _q:
                _bt = max(s.get('tass_score', 0) for s in _q)
                _sir_visible.append((_sg, _bt))
        if not _sir_visible:
            continue
        if args.sort_alphabetical:
            _sir_visible.sort(key=lambda p: p[0].get('toplevelname', 'Unknown'))
        else:
            _sir_visible.sort(key=lambda p: p[1], reverse=True)
        _sir_visible = [sg for sg, _ in _sir_visible]
        _sir_toc_meta = getattr(args, '_input_metadata', {}).get(_sir_sample_name, {})
        ...
        story.append(Spacer(1, 0.05*inch))
        _sir_toc_groups = _sir_visible[:args.max_toc]
        _sir_has_more = len(_sir_visible) > args.max_toc
        _sir_species_links = []
        for _sg in _sir_toc_groups:
            _sp_name = _sg.get('toplevelname', 'Unknown')
            _sp_key = _sg.get('toplevelkey', _sg.get('key', 'unknown'))
            _sp_bm = f"species_{sanitize_bookmark_name(_sir_sample_name)}_{_sp_key}"
            _g_tass, _g_primary, _ = get_species_group_stats(_sg)
            _g_cat = _sg.get('microbial_category', 'Unknown')
            _g_ann = _sg.get('annClass', '')
            _cat_color = get_category_color(_g_cat, _g_ann, alpha=1.0)
            _color_hex = '#{:02x}{:02x}{:02x}'.format(
                int(_cat_color.red * 255), int(_cat_color.green * 255), int(_cat_color.blue * 255))
        ...
        if _sir_has_more:
            ...
            _fh_key = _fh.get('toplevelkey', _fh.get('key', 'unknown'))
            _fh_bm = f"species_{sanitize_bookmark_name(_sir_sample_name)}_{_fh_key}"
            _sir_species_links.append(create_safe_link(
                f"... ({len(_sir_visible) - args.max_toc} more)", _fh_bm, valid_bookmarks))
        story.append(Paragraph(f"→ {', '.join(_sir_species_links)}", indent_style))
        story.append(Spacer(1, 0.02*inch))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(TASS, Classifier Reads, Reads, RPM, Coverage); the right 3 columns contain a "
