...
2. Extract JSON from the dataset (assets/pages.js — the single committed
   demo payload, regenerated by scripts/make_heatmap_boot.py).
...
def main():
    ...
    # ── Data source ─────────────────────────────────────────────────────────
    # assets/pages.js is the one committed demo payload. There is deliberately
    # no fallback: a second dataset in the tree is how the published site ended
    # up silently building from a stale copy.
    if not PAGES_JS.exists():
        sys.exit(
            f"ERROR: dataset not found: {PAGES_JS.relative_to(REPO_ROOT)}\n"
            "Regenerate it with:\n"
