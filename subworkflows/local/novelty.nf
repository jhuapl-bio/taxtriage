...
//
// The backend is chosen by --novelty (params.novelty). By DEFAULT a Pyrodigal ORF-prediction step
// runs first and the predicted nucleotide CDS (.fna) become the query: one whole-genome contig
// becomes one query per gene, so a single near-complete genome contributes several LCA rows (counts)
// instead of one -- closer to a per-gene BLAST view and friendlier to the contig-count-based novelty
// score. Passing --disable_gene skips Pyrodigal and feeds the raw de novo contigs DIRECTLY instead,
// trading granularity for a little less runtime.
//
//   --novelty mmseqs2 : genes (or contigs) -> MMSEQS_TAXONOMY (translated LCA; mmseqs extracts ORFs
//                       internally, so nucleotide input is fine -- predicted genes are just cleaner
//                       and faster than whole contigs)
//   --novelty kaiju   : genes (or contigs) -> KAIJU (greedy translated search; self-translates)
//   --novelty bracken : genes (or contigs) -> KRAKEN2_NOVELTY -> BRACKEN (count-weighted abundance)
//
// --disable_gene OFF (default) : query = PYRODIGAL predicted genes (.fna) over the de novo contigs
// --disable_gene ON            : query = de novo contigs directly (one query per contig)
//
...
workflow NOVELTY {
    ...
    main:
        ...
        // ---------------------------------------------------------------------------------
        // Gene-prediction step (default ON; skip with --disable_gene). Pyrodigal predicts ORFs on
